...
"""Async memory copy operations for LMCache on Kunlun KLX_XPU.

Overrides the torch baseline (:mod:`lmcache.v1.platform.torch_ops`) version to
avoid the ``libcudart`` dependency. On Kunlun KLX_XPU with xmlir, ``libcudart.so``
...
# First Party
from lmcache.klx_xpu_cuda_compat.enums import TransferDirection
...
def lmcache_memcpy_async(
    ...
    """Async memory copy for Kunlun KLX_XPU.
    ...
    if int(direction) == int(TransferDirection.H2D):
        # Host -> Device: src is CPU pointer, dest is KLX_XPU (cuda) pointer.
        ...
        # Use a CUDA stream for async behavior (maps to KLX_XPU stream via xmlir).
        with torch.cuda.stream(torch.cuda.Stream()):
            dest_view.copy_(src_view, non_blocking=True)

    elif int(direction) == int(TransferDirection.D2H):
        # Device -> Host: src is KLX_XPU (cuda) pointer, dest is CPU pointer.
