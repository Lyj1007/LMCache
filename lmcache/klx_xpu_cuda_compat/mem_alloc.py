...
"""Hugepage pinned host allocation helpers for Kunlun KLX_XPU.

Kunlun KLX_XPU uses CUDA-compatible host registration APIs for fast H2D/D2H
transfers, but the torch baseline does not request huge pages.  This module
provides THP-backed pinned host allocation for LMCache CPU buffers when
``local_cpu_use_hugepages`` is enabled.

Host registration is performed in 512 GiB segments (see
``_PIN_SEGMENT_BYTES``) to avoid the Kunlun KLX_XPU driver's 32-bit SGL size
overflow in ``memdescCreate`` (``mem_desc.c:282``).  This allows
registering buffers larger than 512 GiB in a single ``_pin_host_memory``
call.

Vendor ABI note: ``xpu_host_register`` / ``xpu_host_unregister`` and
``libxpurt.so`` are the Kunlun runtime's exported C symbols and are kept
verbatim; only LMCache-side naming uses the ``klx_xpu`` prefix.
"""
...
# Two well-known install paths for libxpurt (the Kunlun runtime); first-found
# wins. Empty on non-KLX_XPU hosts, in which case ``_pin_host_memory`` falls back
# to ``cudaHostRegister`` via torch.cuda.cudart().
_xpurt_paths = [
...
# The Kunlun KLX_XPU driver asserts ``SglSize <= 0xffffffffULL`` in
...
# regions.  Larger pools are registered as multiple 512 GiB segments so the
# host-registration overflow is avoided regardless of pool size.
...
def _pin_host_memory(ptr: int, size: int) -> None:
    """Register a host allocation for KLX_XPU or CUDA DMA.

    Buffers larger than ``_PIN_SEGMENT_BYTES`` are registered in 512 GiB
    segments to avoid the Kunlun KLX_XPU driver's 32-bit SGL size overflow
...
def _alloc_thp_pinned(size: int, numa_id: Optional[int] = None) -> int:
    ...
    logger.info(
        "KLX_XPU pinned host allocation succeeded "
...
def host_register(ptr: int, size: int) -> None:
    ...
    Used by the L1 SHM pool host-registration code (KLX_XPU offload v2 section
    10).  Segmentation is handled internally by ``_pin_host_memory``.
...
def find_pinned_pool_covering(
    ...
    ``.copy_()`` at those sub-segment boundaries so the Kunlun KLX_XPU driver
