...
class ParallelStrategy:
    ...
    n_servers: int = 1
    """
    Number of LMCache servers the adapter is configured to use.
    When n_servers > 1 and the model is NOT MLA, each server owns a
    disjoint subset of TP ranks (sub-world sharding).  For MLA, or
    single-server deployments, this is always 1.
    """

    sub_world_size: int = 0
    """
    The per-server "sub world" size used when sharding KV across
    multiple LMCache servers in the non-MLA case.
    - Non-MLA + n_servers > 1: sub_world_size = actual_world_size // n_servers
      (e.g. TP=16 on 2 servers -> sub_world_size = 8).
    - Otherwise: sub_world_size == kv_world_size (no additional sharding).

    ``0`` is only a placeholder default; callers MUST fill this in
    based on the connector config.  Both the scheduler adapter (LOOKUP
    side) and the worker adapter (STORE/RETRIEVE side) must agree on
    this value so the ObjectKeys generated by
    ``ipc_key_to_object_keys`` line up.
    """

    sub_worker_id: int = 0
    """
    The per-server local worker id for the current rank.
    - Non-MLA + n_servers > 1: sub_worker_id = actual_worker_id % sub_world_size
      (e.g. rank 10 on a 2-server TP=16 setup -> sub_worker_id = 2 on server B).
    - Otherwise: sub_worker_id == kv_worker_id.
    """

    server_side_tp_size: int = 0
    """
    The tp_size value reported to the LMCache server in LOOKUP /
    FREE_LOOKUP_LOCKS requests.  This controls the multi-reader lock
    ``extra_count`` on the server side:
    - MLA: server_side_tp_size = tp_size (so extra_count = tp_size - 1).
    - Non-MLA + n_servers > 1: server_side_tp_size = sub_world_size
      (so each ObjectKey is read by exactly 1 local rank -> extra_count = 0).
    - Non-MLA + single server: server_side_tp_size = tp_size
      (degenerates to world_size, extra_count = 0 as before).
    """

    def __post_init__(self) -> None:
        """Back-compat defaults for callers that don't set the sub fields.

        Older constructors (e.g. the vLLM-bundled connector prior to the
        sub-world change) leave ``sub_world_size`` / ``server_side_tp_size``
        at 0. Detect that case and fall back to the pre-sub-world
        behavior: ``sub_world_size == kv_world_size``,
        ``sub_worker_id == kv_worker_id``,
        ``server_side_tp_size == tp_size``.

        Note: ``sub_worker_id`` cannot be disambiguated by a 0 sentinel
        (0 is a legal worker id), so we tie its "unset" detection to
        ``sub_world_size == 0``. Callers that want sub-world sharding
        must set ``sub_world_size`` (and ``sub_worker_id`` +
        ``server_side_tp_size``) together -- see
        ``_compute_sub_world`` in the connector module.
        """
        if self.sub_world_size == 0:
            # Unset -> fall back to "no sub-world sharding" defaults.
            self.sub_world_size = self.kv_world_size
            self.sub_worker_id = self.kv_worker_id
        if self.server_side_tp_size == 0:
            self.server_side_tp_size = self.tp_size
...
class LMCacheMPSchedulerAdapter:
    def __init__(
        ...
        """
        Args:
            server: The server URL for the LMCache message queue
    ...
    def world_size(self) -> int:
        ...
        When there are multiple LMCache servers and we are NOT in MLA
        mode, we shard KV across servers at the TP-rank granularity.
        Each server is registered with and queried using
        ``sub_world_size`` rather than the full ``kv_world_size``,
        so that ``ipc_key_to_object_keys`` only expands into the
        ``sub_world_size`` ObjectKeys that actually live on this server.
        For MLA or single-server deployments, this equals ``kv_world_size``.
        """
        return self.parallel_strategy.sub_world_size

    @property
    def tp_size(self) -> int:
        """The tp_size value forwarded to the LMCache server.

        See ``ParallelStrategy.server_side_tp_size`` for the rationale:
        we want ``compute_extra_count`` on the server to produce the
        right multi-reader lock count in both MLA and sub-world-sharded
        non-MLA deployments.
        """
        return self.parallel_strategy.server_side_tp_size
...
class LMCacheMPWorkerAdapter:
    ...
    def world_size(self) -> int:
        ...
        Must match ``LMCacheMPSchedulerAdapter.world_size`` so that the
        ObjectKeys produced at STORE time (worker) and LOOKUP time
        (scheduler) refer to the same entries in the server's L1 cache.
        """
        return self.parallel_strategy.sub_world_size

    @property
    def worker_id(self) -> int:
        """Get the per-server (sub) kv worker id.

        For non-MLA + n_servers > 1, this is the worker's index within
        its own server (e.g. rank 10 on server B in a TP=16 / 2-server
        setup -> sub_worker_id = 2).  Keeping worker_id local to the
        server means the server sees a contiguous ``0 .. sub_world_size-1``
        worker id space, which is exactly what
        ``ipc_key_to_object_keys`` expects.
        """
        return self.parallel_strategy.sub_worker_id
