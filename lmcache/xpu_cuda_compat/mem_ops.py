...
"""Async memory copy operations for LMCache on Kunlun XPU.

Overrides the ``python_ops_fallback`` version to avoid the libcudart
dependency. On Kunlun XPU with xmlir, ``libcudart.so`` is not available, so
the fallback's pointer-mode path that uses ``cudaMemcpy`` cannot work. This
implementation converts raw pointers to tensor views via
``_tensor_from_ptr`` and performs copies using PyTorch tensor ops
...
from lmcache.xpu_cuda_compat.enums import TransferDirection
...
def lmcache_memcpy_async(
    ...
    """Async memory copy for Kunlun XPU.

    Handles H2D / D2H pointer-based transfers by converting raw pointers to
    tensor views via ``_tensor_from_ptr`` and using PyTorch's ``.copy_()``
    ...
    if int(direction) == int(TransferDirection.H2D):
        # Host -> Device: src is CPU pointer, dest is GPU pointer.
        ...
        # Use a CUDA stream for async behavior (maps to XPU stream via xmlir).
        with torch.cuda.stream(torch.cuda.Stream()):
            dest_view.copy_(src_view, non_blocking=True)

    elif int(direction) == int(TransferDirection.D2H):
        # Device -> Host: src is GPU pointer, dest is CPU pointer.
