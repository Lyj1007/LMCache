...
"""KLX_XPU ops backend: torch baseline plus Kunlun-specific memory overrides.

On Kunlun KLX_XPU the CUDA-compiled ``lmcache.c_ops`` ``.so`` cannot run, so
:class:`KlxXpuDeviceOps` deliberately stays on the pure torch baseline
(:mod:`lmcache.v1.platform.torch_ops`), whose tensor ops route through
xmlir's CUDA compatibility layer to the KLX_XPU hardware.  It overrides only the
...
class KlxXpuDeviceOps(DeviceOps):
    """DeviceOps for Kunlun KLX_XPU: torch baseline + Kunlun memory overrides."""

    device_type: ClassVar[str] = "klx_xpu"

    def ensure_native(self) -> None:
        """No-op: Kunlun KLX_XPU cannot load the CUDA-compiled ``lmcache.c_ops``.
        ...
        logger.info(
            "KlxXpuDeviceOps: using torch baseline (xmlir) with Kunlun memory "
    ...
    def alloc_hugepage_pinned_ptr(self, size, device_id=0):
        # First Party
        from lmcache.klx_xpu_cuda_compat import mem_alloc
    ...
    def lmcache_memcpy_async(self, *args, **kwargs):
        # First Party
        from lmcache.klx_xpu_cuda_compat import mem_ops
