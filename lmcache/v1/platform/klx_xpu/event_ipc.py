...
"""Kunlun KLX_XPU device-event IPC backend.
...
* :meth:`KlxXpuEventIPCBackend.export_event` **synchronizes the event locally**
...
* :meth:`KlxXpuEventIPCBackend.import_event` returns an already-elapsed sentinel,
...
who blocks; the generic
:class:`~lmcache.v1.multiprocess.modules.lmcache_driven_transfer.LMCacheDrivenTransferContext`
reuses this backend and carries the worker-side paths, so the ordering
semantics stay encapsulated in this single backend.
"""
...
class _CompletedEvent:
    ...
    :meth:`KlxXpuEventIPCBackend.export_event` blocks until the exported event
    ...
    def __repr__(self) -> str:  # pragma: no cover - debug aid
        return "<KlxXpuCompletedEvent>"


class KlxXpuEventIPCBackend:
    """Event IPC backend for Kunlun KLX_XPU, ordered through the message queue.
