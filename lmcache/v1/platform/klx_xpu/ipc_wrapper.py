...
"""Kunlun KLX_XPU KV-cache IPC wrapper.
...
:meth:`KlxXpuPtrIPCWrapper.to_tensor` fails closed if the reconstructed tensor
...
_ORDINAL_UUID_PREFIX = "klx_xpu-ordinal:"


class KlxXpuPtrIPCWrapper(DeviceIPCWrapper):
    """Zero-copy device-pointer wrapper for Kunlun KLX_XPU KV caches.

    Bound to ``device_type="klx_xpu"`` via
    :attr:`~lmcache.v1.platform.klx_xpu.KlxXpuDeviceSpec.ipc_wrapper_cls`, so
    :func:`~lmcache.v1.platform.resolve_kv_wrapper_factory` dispatches to it
    once the reported ``"cuda"`` device type has been normalized to ``"klx_xpu"``
    by :func:`~lmcache.v1.platform._device_detect.normalize_device_type`.
    """

    #: LMCache logical device type this wrapper handles. Note this is ``"klx_xpu"``
    #: even though the tensors themselves report ``device.type == "cuda"``
    #: (xmlir drives Kunlun through ``torch.cuda``).
    device_type: ClassVar[str] = "klx_xpu"

    @classmethod
    def wrap(cls, tensor: torch.Tensor) -> "KlxXpuPtrIPCWrapper":
        """Factory used by
        :func:`~lmcache.v1.platform.resolve_kv_wrapper_factory`.

        Args:
            tensor: A Kunlun KLX_XPU KV-cache tensor.

        Returns:
            A new :class:`KlxXpuPtrIPCWrapper` aliasing ``tensor``.
        """
        return cls(tensor)
    ...
    def __init__(self, tensor: torch.Tensor) -> None:
        ...
        if not tensor.is_contiguous():
            raise ValueError(
                "KLX_XPU pointer transfer requires a contiguous KV-cache tensor; "
        ...
        if data_ptr == 0:
            raise ValueError(
                "KLX_XPU pointer transfer got a null data_ptr; the KV cache is "
    ...
    def to_tensor(self) -> torch.Tensor:
        ...
        if int(tensor.data_ptr()) != self.data_ptr:
            raise RuntimeError(
                "KLX_XPU pointer transfer could not alias worker memory "
