...
"""KPU host-memory pinning backed by the Kunlun runtime (libxpurt).

Delegates to :mod:`lmcache.kpu_cuda_compat.mem_alloc`, which registers host
...
class KpuPinMemoryBackend(PinMemoryBackend):
    """Pin host memory for KPU DMA via the Kunlun host-registration API."""

    def pin_memory(self, ptr: int, size: int, flags: int = 0) -> bool:
        """Register a host region for KPU DMA.
        ...
        try:
            # First Party
            from lmcache.kpu_cuda_compat.mem_alloc import host_register

            host_register(ptr, size)
            return True
        except Exception as exc:
            logger.warning(
                "KPU host_register failed for ptr=%#x size=%d: %s",
                ptr,
                size,
                exc,
            )
    ...
    def unpin_memory(self, ptr: int) -> bool:
        ...
        try:
            # First Party
            from lmcache.kpu_cuda_compat.mem_alloc import host_unregister

            host_unregister(ptr)
            return True
        except Exception as exc:
            logger.warning("KPU host_unregister failed for ptr=%#x: %s", ptr, exc)
            return False

    @property
    def is_pin_supported(self) -> bool:
        """Whether KPU host pinning is supported on this system.
        ...
        try:
            # First Party
            from lmcache.kpu_cuda_compat.mem_alloc import has_kpu_runtime
