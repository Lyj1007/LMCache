...
class VLLMPagedMemKLX_XPUConnectorV2(GPUConnectorInterface):
    """
    ...
    def from_metadata(
        ...
        Returns:
            A new instance of VLLMPagedMemKLX_XPUConnectorV2.
        """
    ...
    def _initialize_pointers(self, kv_caches: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert self.device.type in ("xpu", "cuda"), (
            "Kunlun KLX_XPU runs via xmlir; torch device.type must be xpu or cuda."
        )
        ...
        if self.use_mla:
            if memory_obj.metadata.fmt != MemoryFormat.KV_MLA_FMT:
                raise ValueError(
                    ...
                    " order to be processed by VLLMPagedMemKLX_XPUConnector"
                )
        else:
...
class VLLMPagedMemKLX_XPUConnectorV3(GPUConnectorInterface):
    def __init__(
        self,
        metadata: "LMCacheMetadata",
        device: torch.device,
        ...
        assert device.type in ("xpu", "cuda"), (
            "Kunlun KLX_XPU runs via xmlir; torch device.type must be xpu or cuda."
        )
    ...
    def _initialize_kv_cache_pointers(self):
        ...
        self.init = True
        logger.info("init kv cache pointers success in VLLMPagedMemKLX_XPUConnectorV3")
...
class VLLMBufferLayerwiseKLX_XPUConnector(GPUConnectorInterface):
    def __init__(
        self,
        hidden_dim_size: int,
        num_layers: int,
        ...
        **kwargs,
        ...
        assert use_gpu, "use_gpu must be true in VLLMBufferLayerwiseKLX_XPUConnector"
        assert "dtype" in kwargs, "dtype should be provided to create a GPU buffer."
    ...
    def from_metadata(
        ...
        Returns:
            A new instance of VLLMBufferLayerwiseKLX_XPUConnector.
        """
        # Extract parameters from metadata
        # kv_shape: (num_layer, 2 or 1, chunk_size, num_kv_head, head_size)
        num_layers = metadata.kv_shape[0]
...
class VLLMPagedMemLayerwiseKLX_XPUConnector(GPUConnectorInterface):
    """ """
    ...
    def from_metadata(
        ...
        Returns:
            A new instance of VLLMPagedMemLayerwiseKLX_XPUConnector.
        """
        # Extract parameters from metadata
        # kv_shape: (num_layer, 2 or 1, chunk_size, num_kv_head, head_size)
        num_layers = metadata.kv_shape[0]
...
class SGLangKLX_XPUConnector(GPUConnectorInterface):
    """
...
class SGLangLayerwiseKLX_XPUConnector(GPUConnectorInterface):
    """
    The GPU KV cache should be a list of tensors, one for each layer.
    More specifically, we have:
