...
"""Hugepage pinned host allocation helpers for Kunlun KPU.

Kunlun KPU uses CUDA-compatible host registration APIs for fast H2D/D2H
...
``_PIN_SEGMENT_BYTES``) to avoid the Kunlun KPU driver's 32-bit SGL size
...
verbatim; only LMCache-side naming uses the ``kpu`` prefix.
"""
...
# wins. Empty on non-KPU hosts, in which case ``_pin_host_memory`` falls back
...
# The Kunlun KPU driver asserts ``SglSize <= 0xffffffffULL`` in
...
# ``kpu_transfer`` splits the copy at each 512 GiB boundary as a fallback.
...
def _pin_host_memory(ptr: int, size: int) -> None:
    """Register a host allocation for KPU or CUDA DMA.

    Buffers larger than ``_PIN_SEGMENT_BYTES`` are registered in 512 GiB
    segments to avoid the Kunlun KPU driver's 32-bit SGL size overflow
...
def _alloc_thp_pinned(size: int, numa_id: Optional[int] = None) -> int:
    ...
    logger.info(
        "KPU pinned host allocation succeeded "
...
def host_register(ptr: int, size: int) -> None:
    ...
    Used by the L1 SHM pool host-registration code (KPU offload v2 section
...
def find_pinned_pool_covering(
    ...
    ``.copy_()`` at those sub-segment boundaries so the Kunlun KPU driver
